...
export class Simulador extends Phaser.Scene {
  ...
  createButtons() {
    ...
    this.createButton(250, this.cameras.main.height - 30, "volver", 0.125, () =>
    ...
    this.createTextButton(
      ...
      this.cameras.main.height - 30,
      "Info Areas",
      this.showInfoAreas.bind(this)
    );
    this.createTextButton(
      1100,
      this.cameras.main.height - 30,
      "Reubicar Contenedores",
  ...
  createTextButton(x, y, text, onClick) {
    let button = this.add
      .text(x, y, text, { fontSize: "20px", fill: "#ffffff" })
      .setInteractive();
  ...
  createContainers(data) {
    ...
    // Generate initial positions in the first area
    ...
    fetch("http://localhost:3000/api/cargar")
      ...
      .then((data) => {
        ...
        if (savedData) {
          ...
          this.cuadricula.updateNiveles(); // Update levels after loading container positions
        }
      })
